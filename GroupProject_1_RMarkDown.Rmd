...
```{r}
logisticmodel3 = glm(loan_status ~ number_of_days + principal_balance + amount_funded + borrower_rate + listing_term + prosper_score + total_open_revolving_accounts + real_estate_balance + total_inquiries + was_delinquent_derog, data = listings2, family = "binomial")
...
* We choose **Model 3** as final regression model which explains the variations in default rate because:
  ...
  * The model yields relatively lower **AIC** and higher **McFadden's Pseudo R-squared**, suggesting **Model 3** fits the data well among the three models (not considering Model 1). 
...
Looking at the Confusion matrices, the number of summed "False positives" and "False negatives," is lowest in model 2 (Model 2 = 2,293, Model 3 = 2,375, and Model 4 = 2,444). That said, the scenario that is most damaging is where you predict no default (a 0) and a default happens (a 1). The model that does the best in reducing this scenario is Model 4. As such, while there are very small differences in the models, **Model 4 is our selected model.** 
>>>>>>> 22053882160702271a486a0b2bc0cf253446e7c8
...
4. `real_estate_balance`: **odds ratio** = exp(coefficient of `real_estate_balance`) = `r round(exp(coef(summary(logisticmodel3))["real_estate_balance","Estimate"]), 6)`
